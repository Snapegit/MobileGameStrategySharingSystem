	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyouxigonglve`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyouxigonglve`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussyouxigonglve`, this.ruleForm);
						}else{
							await this.$api.add(`discussyouxigonglve`, this.ruleForm);
						}
						uni.setStorageSync('discussyouxigonglveCleanType',true);
