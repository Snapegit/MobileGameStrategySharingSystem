	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('youxifenlei', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','youxifenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','youxifenlei');
			...
			async init(){
				...
				let res = await this.$api.info('youxifenlei', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussyouxifenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
