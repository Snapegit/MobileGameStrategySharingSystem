...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			<view :style='{"width":"calc(98% - 120rpx)","position":"relative","alignItems":"center","justifyContent":"space-between","display":"flex"}'>
				<view :style='{"backgroundColor":"#f5f5f5","margin":"0 30rpx 0 30rpx","color":"#333333","alignItems":"center","borderRadius":"40rpx","flex":"1","display":"flex","lineHeight":"64rpx","fontSize":"24rpx","height":"64rpx"}'>
					...
					<input v-model="searchForm.youximingcheng" type="text" placeholder="游戏名称" :style='{"background":"transparent","height":"100%"}'></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"40rpx 24rpx 24rpx 24rpx","margin":"20rpx 0 0 0","flex":"1","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px #00000020","padding":"0 0 20rpx 0","margin":"0 0 40rpx 0","borderColor":"#cccccc","flexWrap":"wrap","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"dashed","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
				  <image :style='{"margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"200rpx","height":"200rpx","order":"2"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.youxitupian)" :src="product.youxitupian.split(',')[0]"></image>
				  <image :style='{"margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"200rpx","height":"200rpx","order":"2"}' mode="aspectFill" class="listmpic" v-else :src="product.youxitupian?baseUrl+product.youxitupian.split(',')[0]:''"></image>

				  <view class="listmr" :style='{"width":"calc(100% - 240rpx)","padding":"0 0 0 0","margin":"20rpx 0 0 0","height":"auto"}'>
					<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx 0 20rpx","margin":"0 0 12rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"60rpx","fontSize":"28rpx","textOverflow":"ellipsis","height":"60rpx"}'>{{product.youximingcheng}}</view>
					<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx 0 20rpx","margin":"0 0 12rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"60rpx","fontSize":"28rpx","textOverflow":"ellipsis","height":"60rpx"}'>发布日期:{{product.faburiqi}}</view>
				  </view>

					<view :style='{"width":"100%","padding":"40rpx 20rpx 40rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('youxigonglve','修改')) || (!userid && isAuthFront('youxigonglve','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('youxigonglve','删除')) || (!userid && isAuthFront('youxigonglve','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('youxigonglve','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('youxigonglve','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
	...
	<view :style='{"width":"80%","padding":"40rpx 20rpx","background":"#d1f1ed"}' class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
		<view :style='{"width":"100%","padding":"20rpx 20rpx 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
			<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>角色扮演</view>
			<input :style='{"width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","color":"#666666","background":"#ffffff","height":"60rpx"}' placeholder="请输入角色扮演" v-model="searchForm.jiaosebanyan">
		</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.youximingcheng="";
				this.searchForm.jiaosebanyan="";
			},
			//类别搜索
        ...
        params['sort'] = 'thumbsupnum';
        params['order'] = 'desc';


				if(this.searchForm.youximingcheng){
					params['youximingcheng'] = '%' + this.searchForm.youximingcheng + '%'
				}
				if(this.searchForm.jiaosebanyan){
					params['jiaosebanyan'] = '%' + this.searchForm.jiaosebanyan + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`youxigonglve`, params);
                } else {
                    res = await this.$api.list(`youxigonglve`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('youxigonglve', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.youximingcheng){
					searchForm['youximingcheng'] = '%' + this.searchForm.youximingcheng + '%'
				}
				if(this.searchForm.jiaosebanyan){
					searchForm['jiaosebanyan'] = '%' + this.searchForm.jiaosebanyan + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`youxigonglve`, searchForm);
                } else {
                    res = await this.$api.list(`youxigonglve`, searchForm);
                }
				// 如果是第一页数据置空
